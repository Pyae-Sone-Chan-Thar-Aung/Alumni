-- =============================================================================
-- UIC Alumni Portal - Complete Database Fixes for Failing Tests
...
DROP POLICY IF EXISTS "Allow public registration insertions" ON pending_registrations;
...
-- =============================================================================
-- Fix 7: Enable RLS on specific tables only (skip views)
-- =============================================================================

-- Enable RLS on users table (if it's a table)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND information_schema.tables.table_name = 'users'
        AND table_type = 'BASE TABLE'
    ) THEN
        ALTER TABLE users ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE 'Enabled RLS on table: users';
    ELSE
        RAISE NOTICE 'Skipped users (not a base table)';
    END IF;
END $$;

-- Enable RLS on user_profiles table (if it's a table)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND information_schema.tables.table_name = 'user_profiles'
        AND table_type = 'BASE TABLE'
    ) THEN
        ALTER TABLE user_profiles ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE 'Enabled RLS on table: user_profiles';
    ELSE
        RAISE NOTICE 'Skipped user_profiles (not a base table)';
    END IF;
END $$;

-- Enable RLS on pending_registrations table (if it's a table)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND information_schema.tables.table_name = 'pending_registrations'
        AND table_type = 'BASE TABLE'
    ) THEN
        ALTER TABLE pending_registrations ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE 'Enabled RLS on table: pending_registrations';
    ELSE
        RAISE NOTICE 'Skipped pending_registrations (not a base table)';
    END IF;
END $$;

-- Enable RLS on job_opportunities table (if it's a table)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND information_schema.tables.table_name = 'job_opportunities'
        AND table_type = 'BASE TABLE'
    ) THEN
        ALTER TABLE job_opportunities ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE 'Enabled RLS on table: job_opportunities';
    ELSE
        RAISE NOTICE 'Skipped job_opportunities (not a base table)';
    END IF;
END $$;

-- Enable RLS on tracer_study_responses table (if it's a table)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' 
        AND information_schema.tables.table_name = 'tracer_study_responses'
        AND table_type = 'BASE TABLE'
    ) THEN
        ALTER TABLE tracer_study_responses ENABLE ROW LEVEL SECURITY;
        RAISE NOTICE 'Enabled RLS on table: tracer_study_responses';
    ELSE
        RAISE NOTICE 'Skipped tracer_study_responses (not a base table)';
    END IF;
END $$;

-- =============================================================================
-- Fix 8: Create policies for actual tables only (without custom functions)
-- =============================================================================
...
-- Tracer study responses policies (only if it's a table, not a view) - using inline admin check
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables 
...
BEGIN
  ...
  WHERE table_schema = 'public' 
  AND table_type = 'BASE TABLE'
  AND information_schema.tables.table_name IN ('users', 'user_profiles', 'pending_registrations', 'news', 
                     'job_opportunities', 'tracer_study_responses', 'gallery_albums', 'gallery_images');
  
  SELECT COUNT(*) INTO view_count
  FROM information_schema.views 
  WHERE table_schema = 'public' 
  AND information_schema.views.table_name IN ('users', 'user_profiles', 'pending_registrations', 'news', 
                     'job_opportunities', 'tracer_study_responses', 'gallery_albums', 'gallery_images');
